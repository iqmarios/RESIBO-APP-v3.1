/* Resibo App v3.6.2 — Codex-Enhanced
   Optional OpenCV load; falls back to Canvas if unavailable.
*/
(()=>{
  ...
  // ======= state =======
  let CSV_ROWS=[];          // verification data
  let files=[];             // [{id,name,urlOriginal,urlProcessed,rotation,ocr}]
  let currentId=null;
  let ocrTextAll='';
  ...
  const viewer={scale:1,rot:0,ox:0,oy:0,
    ...
    filters(){ const bs=+$('#v-bright').value/100, cs=+$('#v-contrast').value/100; $('#v-before').style.filter=`brightness(${bs}) contrast(${cs})`; $('#v-after').style.filter=`brightness(${bs}) contrast(${cs})`; }
  };
  ...
  function runSelfTest(){
    ...
    const tests=[
      ...
      ['OpenCV (optional)',()=> !!window.cv && !!window.cv.Mat]
    ];
    host.innerHTML='';
    tests.forEach(([name,fn])=>{
      const ok=!!fn();
      const div=document.createElement('div'); div.className='check '+(ok?'ok':'bad');
      div.textContent=`${name} — ${ok?'OK':'N/A'}`;
  ...
  async function testCSV(url){
    ...
    try{
      ...
      setPill(pill,'CSV error: '+err.message,'warn'); return false;
    }
  }
  const normalizeTIN=s=>(s||'').replace(/[\s-]/g,'');
  async function handleVerify(){
    const url=$('#csvUrl').value.trim() || localStorage.getItem(LS.CSV)||'';
    const pill=$('#verify-status');
    if(!url){ setPill(pill,'CSV URL missing','warn'); return; }
    localStorage.setItem(LS.CSV,url);
    const ok=await testCSV(url); if(!ok){ setPill(pill,'CSV fetch failed','warn'); return; }
    ...
    const rec=CSV_ROWS.find(r => (r.code||'').toLowerCase()===code.toLowerCase() && (r.gmail||'').toLowerCase()===gmail.toLowerCase());
    ...
    if((rec.expiry_date||'')<today){ setPill(pill,`Expired ${rec.expiry_date}`,'warn'); return; }
    if(normalizeTIN(rec.tin)!==normalizeTIN(tin)){ setPill(pill,'TIN mismatch','warn'); return; }

    localStorage.setItem(LS.SESSION, JSON.stringify({code,name,tin,gmail,expiry:rec.expiry_date,savedAt:new Date().toISOString()}));
  ...
  // ======= OpenCV optional loader =======
  let HAS_CV=false;
  async function loadOpenCV(){
    if(window.cv && window.cv.Mat){ HAS_CV=true; return true; }
    // only try if the file exists in /libs/
    await new Promise((resolve)=>{
      const s=document.createElement('script');
      s.src='libs/opencv.js'; s.async=true;
      s.onload=()=>resolve();
      s.onerror=()=>resolve();
      document.head.appendChild(s);
      setTimeout(resolve, 3000); // soft timeout
    });
    // try to wait for runtime init
    if(window.cv && typeof cv['onRuntimeInitialized'] === 'function'){
      await new Promise(r=>{ const to=setTimeout(r,1500); cv['onRuntimeInitialized']=()=>{ clearTimeout(to); r(); }; });
    }
    HAS_CV = !!(window.cv && window.cv.Mat);
    return HAS_CV;
  }

  // ======= preprocess (CV or Canvas) =======
  $('#btn-preprocess-basic')?.addEventListener('click',()=>preprocessAll('basic'));
  $('#btn-preprocess-strong')?.addEventListener('click',()=>preprocessAll('strong'));
  $('#btn-preprocess-ultra')?.addEventListener('click',()=>preprocessAll('ultra'));

  async function preprocessAll(preset){
    if(!files.length){ setPill($('#ocr-status'),'No images','warn'); return; }
    setPill($('#ocr-status'), HAS_CV ? 'Preprocessing (OpenCV)…' : 'Preprocessing (Canvas)…');
    const boost=$('#chk-smallprint')?.checked;
    if(HAS_CV) for(const f of files){ f.urlProcessed=await processCV(f.urlOriginal,preset,boost); }
    else       for(const f of files){ f.urlProcessed=await processCanvas(f.urlOriginal,preset,boost); }
    setPill($('#ocr-status'),'Preprocess done','ok');
    renderThumbs(); if(currentId) showViewer(currentId);
  }

  // --- Canvas fallback (good tiny-print tunings) ---
  async function processCanvas(url,preset='strong',boostSmall=false){
    return new Promise((resolve)=>{
      ...
      img.onload=()=>{
        ...
        const ctx=c.getContext('2d'); ctx.imageSmoothingEnabled=true; ctx.drawImage(img,0,0,w,h);
        let im=ctx.getImageData(0,0,w,h), data=im.data;

        // grayscale
        for(let i=0;i<data.length;i+=4){ const r=data[i],g=data[i+1],b=data[i+2]; const y=r*0.299+g*0.587+b*0.114; data[i]=data[i+1]=data[i+2]=y; }

        // contrast
        const contrast=(preset==='ultra'?1.5:(preset==='strong'?1.3:1.15));
        const gain=(259*(contrast*255+255))/(255*(259-contrast*255));
        for(let i=0;i<data.length;i+=4){ const y=data[i]; const v=Math.max(0,Math.min(255,gain*(y-128)+128)); data[i]=data[i+1]=data[i+2]=v; }

        // local mean threshold
        const grid=(preset==='ultra'?28:(preset==='strong'?40:56));
        const out=new Uint8ClampedArray(w*h*4);
        for(let gy=0;gy<h;gy+=grid){
          for(let gx=0;gx<w;gx+=grid){
            let sum=0,cnt=0; const gy2=Math.min(gy+grid,h), gx2=Math.min(gx+grid,w);
            for(let y=gy;y<gy2;y++){ let idx=(y*w+gx)*4; for(let x=gx;x<gx2;x++,idx+=4){ sum+=data[idx]; cnt++; } }
            const mean=sum/(cnt||1)-(preset==='ultra'?6:8);
            for(let y=gy;y<gy2;y++){ let idx=(y*w+gx)*4; for(let x=gx;x<gx2;x++,idx+=4){ const v=data[idx]<mean?0:255; out[idx]=out[idx+1]=out[idx+2]=v; out[idx+3]=255; } }
          }
        }
  ...
  // --- OpenCV path (if available; mild deskew + CLAHE + adaptive) ---
  async function processCV(url,preset='strong',boostSmall=false){
    return new Promise((resolve)=>{
      const img=new Image(); img.crossOrigin='anonymous';
      img.onload=()=>{
        const sc=boostSmall?2.2:1.6, w=Math.round(img.naturalWidth*sc), h=Math.round(img.naturalHeight*sc);
        ...
        // CLAHE
        const clahe = cv.createCLAHE(preset==='ultra'?3.0:2.0, new cv.Size(8,8)); clahe.apply(gray, gray); clahe.delete();

        // blur & threshold
        if(preset!=='basic') cv.medianBlur(gray, gray, 3);
        let bw=new cv.Mat();
        const block = preset==='ultra'? 21 : 31;
        cv.adaptiveThreshold(gray,bw,255,cv.ADAPTIVE_THRESH_GAUSSIAN_C,cv.THRESH_BINARY, block, preset==='ultra'?8:10);

        // quick deskew by Hough (optional, not failing if empty)
        try{
          let edges=new cv.Mat(); cv.Canny(bw,edges,50,150,3,false);
          let lines=new cv.Mat(); cv.HoughLines(edges,lines,1,Math.PI/180, w*0.6);
          if(lines.rows>0){
            // average angle around 0 or 90
            let ang=0, cnt=0;
            for(let i=0;i<lines.rows;i++){ const theta=lines.data32F[i*2+1]; let deg=(theta*180/Math.PI); if(deg>90) deg-=180; if(Math.abs(deg)<45){ ang+=deg; cnt++; } }
            if(cnt){ ang/=cnt; const center=new cv.Point(bw.cols/2,bw.rows/2); const M=cv.getRotationMatrix2D(center,ang,1); let rot=new cv.Mat(); cv.warpAffine(bw,rot,M,new cv.Size(bw.cols,bw.rows),cv.INTER_LINEAR,cv.BORDER_CONSTANT,new cv.Scalar(255,255,255,255)); bw.delete(); bw=rot; }
          }
        ...
        // output
        cv.imshow(canvas,bw);
        const urlOut=canvas.toDataURL('image/png',0.95);
  ...
  async function runOCR(){
    ...
    renderThumbs(); $('#step3').scrollIntoView({behavior:'smooth'});
  }

  // ======= mapping (robust) =======
  const norm=s=>(s||'').toLowerCase().replace(/\s+/g,' ').trim();
  function levenshtein(a,b){ a=a||''; b=b||''; const m=a.length,n=b.length; const dp=Array.from({length:m+1},()=>Array(n+1).fill(0)); for(let i=0;i<=m;i++) dp[i][0]=i; for(let j=0;j<=n;j++) dp[0][j]=j; for(let i=1;i<=m;i++){ for(let j=1;j<=n;j++){ const c=a[i-1]===b[j-1]?0:1; dp[i][j]=Math.min(dp[i-1][j]+1,dp[i][j-1]+1,dp[i-1][j-1]+c); } } return dp[m][n]; }
  function fuzzyIncludes(text,label,maxDist=2){ text=norm(text); label=norm(label); if(text.includes(label)) return true; const w=text.split(/\s+/), L=label.split(/\s+/); for(let i=0;i<=w.length-L.length;i++){ const win=w.slice(i,i+L.length).join(' '); if(levenshtein(win,label)<=maxDist) return true; } return false; }
  ...
  function mapOCRToFields(raw){
    ...
    if(!docType){ const hasOR=lines.some(l=>fuzzyIncludes(l,'official receipt')||/\bOR\b/.test(l)); const hasSI=lines.some(l=>fuzzyIncludes(l,'sales invoice')||/\bSI\b/.test(l)); docType=hasOR?'Official Receipt':(hasSI?'Sales Invoice':(docNo?'Receipt':'')); }
  ...
  // ======= simple overlay (visual aid; grid; clickable) =======
  function drawOverlay(){
    ...
    OVERLAY.shapes.forEach(s=>{
      ...
      ctx.globalAlpha=0.18; ctx.fillStyle=s.color; ctx.fillRect(s.x,s.y,s.w,s.h); ctx.globalAlpha=1;
    });
  }
  ...
  $('#btn-parse-table')?.addEventListener('click', ()=>{
    ...
    const img=( $('#toggle-before-after')?.checked ? $('#v-before') : $('#v-after') );
    const w=img.naturalWidth, h=img.naturalHeight; if(!w||!h){ setPill($('#save-status'),'Image not ready','warn'); return; }
    OVERLAY.imgW=w; OVERLAY.imgH=h; OVERLAY.shapes=[];
    const rows=6, cols=4, palette=['#1e3a8a','#065f46','#854d0e','#7f1d1d'];
    ...
    drawOverlay();
    setPill($('#save-status'),'Overlay ready (visual grid); items come from OCR text.','ok');
  });
  $('#v-overlay')?.addEventListener('click',(e)=>{
    ...
    const scaleX=(OVERLAY.imgW||cvs.width)/rect.width, scaleY=(OVERLAY.imgH||cvs.height)/rect.height;
    ...
    const hit=OVERLAY.shapes.find(s=>x>=s.x && x<=s.x+s.w && y>=s.y && y<=s.y+s.h); if(!hit) return;
  ...
  (function wireViewer(){
    ...
    canv.addEventListener('wheel',(e)=>{ e.preventDefault(); const v=Math.max(30,Math.min(600, +$('#v-zoom-slider').value+(e.deltaY<0?12:-12))); $('#v-zoom-slider').value=v; viewer.scale=v/100; viewer.apply(); },{passive:false});
    canv.addEventListener('mousedown',(e)=>{ panning=true; sx=e.clientX; sy=e.clientY; canv.style.cursor='grabbing'; });
    addEventListener('mouseup',()=>{ panning=false; canv.style.cursor='default'; });
    addEventListener('mousemove',(e)=>{ if(!panning) return; viewer.ox+=(e.clientX-sx); viewer.oy+=(e.clientY-sy); sx=e.clientX; sy=e.clientY; viewer.apply(); });
    ...
    canv.addEventListener('touchmove',(e)=>{ e.preventDefault(); if(e.touches.length===2 && last!=null){ const d=dist(e.touches[0],e.touches[1]); const v=Math.max(30,Math.min(600, +$('#v-zoom-slider').value+(d-last)/2)); $('#v-zoom-slider').value=v; viewer.scale=v/100; viewer.apply(); last=d; } else if(e.touches.length===1){ viewer.ox+=(e.touches[0].clientX-sx); viewer.oy+=(e.touches[0].clientY-sy); sx=e.touches[0].clientX; sy=e.touches[0].clientY; viewer.apply(); } },{passive:false});
    canv.addEventListener('touchend',()=>{ last=null; });
    function dist(a,b){ const dx=a.clientX-b.clientX, dy=a.clientY-b.clientY; return Math.hypot(dx,dy); }

    $('#v-zoom-slider').addEventListener('input',(e)=>{ viewer.scale=+e.target.value/100; viewer.apply(); });
    $('#v-zoom-in').addEventListener('click',()=>{ const v=Math.max(30,Math.min(600,+$('#v-zoom-slider').value+12)); $('#v-zoom-slider').value=v; viewer.scale=v/100; viewer.apply(); });
    $('#v-zoom-out').addEventListener('click',()=>{ const v=Math.max(30,Math.min(600,+$('#v-zoom-slider').value-12)); $('#v-zoom-slider').value=v; viewer.scale=v/100; viewer.apply(); });
  ...
  ['#f_gross','#f_vat','#f_disc'].forEach(sel=>{
    $(sel)?.addEventListener('input',()=>{ const g=+($('#f_gross').value||0), v=+($('#f_vat').value||0), d=+($('#f_disc').value||0); $('#f_total').value=(g+v-d).toFixed(2); });
  });
  ...
  $('#btn-top')?.addEventListener('click',()=>scrollTo({top:0,behavior:'smooth'}));
  ...
  (async function boot(){
    ...
    // attempt OpenCV; silently fall back if not available
    const ok=await loadOpenCV();
    setPill($('#ocr-status'), ok? 'OpenCV ready' : 'Canvas mode (OpenCV optional)','ok');
  })();
