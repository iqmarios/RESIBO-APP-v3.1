...
(() => {
  ...
  els.verifyForm.addEventListener('submit', async (e) => {
    ...
    if (!access_code || !name || !tin || !gmail) {
      els.verifyMsg.textContent = 'Please fill Access Code, Name, TIN, and Gmail.';
    ...
    try {
      ...
      els.verifyMsg.textContent = `CSV load error: ${String(err.message || err)}`;
      els.verifyMsg.className = 'msg error';
      return;
    }
  ...
  async function onFileAction(e) {
    ...
    if (act === 'remove') {
      ...
      state.records = state.records.filter(r => r.file_id !== id);
      renderFileList(); renderRecords();
      return;
    }
    ...
    if (act === 'preprocess') {
      if (f.type === 'application/pdf') {
        ...
        img.onload = () => {
          ...
          const out = preprocessCanvas(cnv);
          f.dataURL = out.toDataURL('image/jpeg', 0.92);
          renderFileList();
        };
        img.src = f.dataURL;
      }
    ...
    if (act === 'ocr') {
      let canvases = [];
      if (f.type === 'application/pdf' && f.pages?.length) {
        canvases = f.pages;
      } else {
        const cnv = await imageToCanvas(f.dataURL);
        canvases = [preprocessCanvas(cnv)];
      }
      ...
      const idx = state.records.findIndex(r => r.file_id === f.id);
  ...
  function preprocessCanvas(inputCanvas) {
    const cnv = document.createElement('canvas');
    cnv.width = inputCanvas.width; cnv.height = inputCanvas.height;
  ...
  function renderRecords() {
    ...
    for (const r of state.records) {
      ...
      btnValidate.addEventListener('click', () => {
        ...
        alert(errors.length ? `Please fix:\n- ${errors.join('\n- ')}` : 'Looks good!');
      });
  ...
  function setRecField(rec, keyPath, value) {
    const parts = keyPath.split('.'); let cur = rec;
  ...
  $('btnZipExport').addEventListener('click', async () => {
    if (els.inpConfirmAll?.value !== 'yes') { toast('Please confirm: Have you corrected all receipt data? Select "Yes".', 'error'); return; }
    const zip = new JSZip();
    for (const f of state.files) zip.file(`files/${f.name}`, f.blob);
    const csv = buildCSV(state.records);
    const mani = buildManifest(state.records);
  ...
  $('btnEmailExport').addEventListener('click', async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      toast('Email export sent.');
    } catch (err) {
      toast(`Email export failed: ${String(err.message||err)}`, 'error');
    }
  });

  function buildCSV(records) {
    const headers = [
      'FILE_NAME','MIME','PAGES',
    ...
    for (const r of records) {
      const vals = [
        r.file_name, r.file_meta?.mime || '', r.file_meta?.pages || '',
        r.seller?.name || '', r.seller?.tin || '',
  ...
  (function initFromStorage() {
    ...
    if (session && session.schema === SCHEMA_VERSION) {
      ...
      $('inpTIN').value = session.tin || '';
      $('inpGmail').value = session.gmail || '';
      els.sectionCapture.hidden = false;
      els.sectionReview.hidden = false;
      els.sectionExport.hidden = false;
      els.sectionPrivacy.hidden = false;
